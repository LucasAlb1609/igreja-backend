...
# SEU MODELO User E Filho CONTINUAM AQUI (sem alterações)
# ...
...
# --- NOVO MODELO PARA GERENCIAR OS DOCUMENTOS ---
class ModeloDocumento(models.Model):
    """
    Modelo para gerenciar os templates de certificados e declarações.
    """
    nome = models.CharField(
        max_length=100, 
        verbose_name="Nome do Documento",
        help_text="Ex: Declaração de Membro, Certificado de Batismo"
    )
    
    arquivo_template = models.CharField(
        max_length=200, 
        verbose_name="Caminho do Arquivo de Template",
        help_text="Ex: certificados/declaracao_membro.html. O caminho é relativo à pasta 'templates' do app."
    )
    
    # Este campo armazena os papéis permitidos como um texto separado por vírgulas.
    # Ex: "membro,secretario"
    papeis_permitidos = models.CharField(
        max_length=100,
        verbose_name="Papéis Permitidos",
        help_text="Separe os papéis por vírgula. Ex: membro,secretario. Papéis disponíveis: congregado, membro, secretario."
    )

    def __str__(self):
        return self.nome

    def get_papeis_list(self):
        """Retorna uma lista dos papéis permitidos."""
        return [papel.strip() for papel in self.papeis_permitidos.split(',') if papel]
